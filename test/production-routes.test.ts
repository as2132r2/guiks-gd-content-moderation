 ...
 * 生产构建的路由面。
 *
 * 遗留的六组路由（策略 / 靶场 / 红队 / runtime / 报告 / 内存态播种）是 AuditGate
 * 时代的本地工具，其中几个每次调用都会消耗真实模型额度——所以生产下根本不挂载。
 * （它们在 demo 下现在也要登录并持有 `audit:read`，见
 * [legacy-routes-auth.test.ts](legacy-routes-auth.test.ts)；**不挂载**仍然是比
 * 鉴权更强的一道，两道都要。）
 *
