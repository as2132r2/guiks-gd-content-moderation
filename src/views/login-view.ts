...
export function renderLogin(options: { demoLoginEnabled: boolean; next?: string }): string {
  ...
  <title>登录 · 把关人</title><style>
  :root{color-scheme:light;--ink:#202823;--muted:#6d756f;--green:#1f6f4a;--line:#d9ded9;--paper:#f3f1e9}
  *{box-sizing:border-box}body{margin:0;min-height:100vh;display:grid;place-items:center;background:var(--paper);color:var(--ink);font-family:"Microsoft YaHei",sans-serif}
  main{width:min(900px,92vw);display:grid;grid-template-columns:1fr 1fr;background:#fff;border:1px solid var(--line);border-radius:18px;overflow:hidden;box-shadow:0 20px 60px #24372b18}
  section{padding:38px}.brand{background:#183c2c;color:#fff;display:flex;flex-direction:column;justify-content:center}.brand small{opacity:.68;letter-spacing:.08em}.brand h1{font-size:34px;margin:12px 0}.brand p{line-height:1.8;opacity:.82}
  h2{margin:0 0 8px;font-size:20px}.quick>p{color:var(--muted);font-size:13px}.persona{display:block;width:100%;text-align:left;border:1px solid var(--line);background:#fff;border-radius:10px;padding:12px 14px;margin:9px 0;cursor:pointer}.persona:hover{border-color:var(--green);background:#f6fbf8}.persona strong,.persona span{display:block}.persona span{font-size:12px;color:var(--muted);margin-top:4px}
