 ...
 * 「邪教」「偷拍设备」「虚开发票」全都是台里正经在写的稿子（破获制毒窝点、
...
/**
 * 基本量词。**顺序即优先级，长的必须排前面**——「米」排到「平方米」前面，
 * 12 万平方米会被截成「12 万平」，比对时就成了另一个数。
 */
const UNITS = '人次|平方米|千米|公里|元|人|户|个|家|米|吨|％|%';
 ...
 *
 * **量级后面的量词要一起吃进来。** 早前这里写的是 `亿元|万元|亿|万|…`，于是
 * 「3.2 亿人」只抽出「3.2 亿」、「8.6 万人」只抽出「8.6 万」——量词被吃掉之后，
 * 原通稿的「3.2 亿元」和生成稿的「3.2 亿人」在比对里长得一模一样，**一处把
 * 金额说成人数的幻觉会被静静放过**。所以量级分支排在前面，并带上可选量词。
 */
// String.raw 不是讲究：普通模板串里 `\d` 会被 JS 当成未知转义、把反斜杠吞掉，
// 正则会变成 `d+(?:.d+)?s*…` —— 编译通过、类型正确、一个数字也匹配不到。
export const NUMBER_PATTERN = new RegExp(
  String.raw`\d+(?:\.\d+)?\s*(?:[亿万]\s*(?:${UNITS})?|${UNITS})` +
    String.raw`|\d{4}\s*年` +
    String.raw`|\d{1,2}\s*月\d{1,2}\s*日`,
  'g',
);
