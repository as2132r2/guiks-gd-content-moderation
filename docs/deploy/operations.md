...
1. **建 5 个账号**（`zhangmin` / `lijianguo` / `wangzhiyuan` / `stationadmin` / `chenxue`），口令取 `SEED_PASSWORD`
2. **清空旧稿件，重建 15 篇**——前 7 篇是教学样本，覆盖准入三档、主链各阶段与一次退回；后 8 篇是本底数据
...
完成：稿件 15 篇，已签发 10，留痕 315 条，全台 AI 参与度 95.0%
```
...
**这组数字是幂等的**——重复播种应该得到完全一样的结果。对不上说明词表或 mock 变过了。
